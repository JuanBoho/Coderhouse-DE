...
def create_tables(conn, engine, schema):
    ...
    :param Connection conn. Instancia del objeto Connnection de sqlalchemy.
    :param Engine engine. Instacia del objeto Engine de sqlalchemy.
    :param str schema. Nombre del schema donde se crearán las tablas.
    """
...
def load_aq_data(data, conn, table="", schema="", check_field="",check_field_two=""):
    ...
    try:
        ...
        extra_check = ";" if check_field_two == "" else f" AND {table}.{check_field_two} = {table}_stg;"
        sql = f"""BEGIN;
            DELETE FROM {table}
            USING {table}_stg
...
def load_locations_data(config_file):
    """
    ...
    try: 
        ...
        # Carga info sobre locaciones
        load_aq_data(locations_df, conn, "locations", "juanbohorquez_ar_coderhouse", "location_id")
    except Exception as e:
        log.error('Error al cargar los datos de locaciones')
        raise e


def load_aq_measuraments_data(config_file):
    """
    Obtiene los datos de la API para las locaciones y los carga en Redshift
    """
    try: 
        ...
        # Carga info sobre locaciones
        load_aq_data(measurements_df, conn, "aq_measuraments", "juanbohorquez_ar_coderhouse", "location_id", "date")
    except Exception as e:
        log.error('Error al cargar los datos de locaciones')
        raise e
